...
from dataset_trans import Dataset, EncoderDecoderCollator, EvaluationMetrics
import wandb
import csv
import shutil
import re
...
def parse_bleu_results(bleu_score_obj):  
    data = {
        "score": float(bleu_score_obj.score),
        "1-gram": bleu_score_obj.precisions[0],
        "2-gram": bleu_score_obj.precisions[1],
        "3-gram": bleu_score_obj.precisions[2],
        "4-gram": bleu_score_obj.precisions[3],
        "bp": bleu_score_obj.bp,
        "ratio": bleu_score_obj.sys_len / bleu_score_obj.ref_len,
        "hyp_len": bleu_score_obj.sys_len,
        "ref_len": bleu_score_obj.ref_len
    }
    return data
...
def parse_arguments():
    ...
    parser.add_argument("--output_dir", default="translation", type=str)
    parser.add_argument("--task_prefix", default="oversæt fra dansk til svensk: ", type=str)
    ...
    parser.add_argument("--wandb_prefix", default="translation_test", type=str)
...
if __name__ == "__main__":
    ...
    seed_results = []
    train_set = Dataset(args.train_file, args.source_lang, args.target_lang)
    valid_set = Dataset(args.validation_file, args.source_lang, args.target_lang)
    test_set = Dataset(args.test_file, args.source_lang, args.target_lang)
    ...
    for seed in seed_list:
                    ...
                    with torch.no_grad():
                        ...
                        for i, batch in enumerate(tqdm_bar_eval):
                            ...
                            with torch.cuda.amp.autocast(args.mixed_precision):
                                ...
                                metric.add_batch(generated=decoded_preds, groundtruth=[decoded_labels])
                        result = metric.compute_scores()
                        bleu_score = result.score
                        formatted_result = parse_bleu_results(result)
                        # if args.load_best_at_the_end:
                        if bleu_score > best_eval_score["bleu"]:
                            ...
                            best_eval_score["bleu"] = bleu_score
                            best_eval_score["epoch"] = epoch
                            print("Updated best_eval_score:", best_eval_score)
                        tqdm.write(f"Eval result: {formatted_result}")
                        if args.use_wandb: 
                            wandb.log({"eval/" + k: v for k,v in formatted_result.items()}, step=total_train_steps)
        ...
        result = metric.compute_scores()
        tqdm.write(f"Test result: {result}")
        formatted_result = parse_bleu_results(result)
        seed_results.append(formatted_result)
        tqdm.write(f"Test BLEU score: {formatted_result}")
        if args.use_wandb:
            wandb.log({"test/" + k: v for k,v in formatted_result.items()})
            wandb.finish()
    # Calculate mean
    mean_results = {
    ...
    with open(f"results_{args.output_dir}_{args.model.split('/')[-1]}.txt", 'a') as f:
        for run_time, seed_result in enumerate(seed_results):
            f.write(f"Run: {run_time}\tBLEU: {seed_result}\n")
        f.write(f"Mean:\tBLEU {mean_results}\n")
